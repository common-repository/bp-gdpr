...
if ( ! class_exists( '\BuddyBoss\BpGdpr\MessageExport' ) ) :

	final class MessageExport extends Export {
		...
		public static function instance() {
			...
			if ( null === $instance ) {
				$instance = new MessageExport();
				$instance->setup( "bp_message", __( "Private Messages", 'buddyboss-bp-gdpr' ) );
			}
		...
		function process_data( $user, $page, $email_address = false ) {
			...
			foreach($data_items["items"] as $item) {
				...
				$group_label = __("Message Threads & Replies",'buddyboss-bp-gdpr');
				...
				$data = array(
					array("name"=>__('Message Subject','buddyboss-bp-gdpr'), "value" => $item->subject),
					array("name"=>__('Message Content','buddyboss-bp-gdpr'), "value" => $item->message),
					array("name"=>__('Created Date (GMT)','buddyboss-bp-gdpr'), "value" => $item->date_sent),
					array("name"=>__('Message Recipients','buddyboss-bp-gdpr'), "value" => $recipients),
					array("name"=>__('Thread URL','buddyboss-bp-gdpr'), "value" => $permalink),
				);
		...
		function messages_recipients($items) {
			...
			$get_results = $wpdb->get_results($wpdb->prepare("SELECT *FROM {$table} WHERE thread_id IN ({$ids_in})",$thread_ids));
