...
if ( ! class_exists( '\BuddyBoss\BpGdpr\GroupExport' ) ) :

	final class GroupExport extends Export {
		...
		public static function instance() {
			...
			if ( null === $instance ) {
				$instance = new GroupExport();
				$instance->setup( "bp_groups", __( "Groups", 'buddyboss-bp-gdpr' ) );
			}
		...
		function process_data( $user, $page, $email_address = false ) {
			...
			foreach($data_items["items"] as $item) {
				...
				$group_id    = "bp_groups";
				$group_label = __("Groups",'buddyboss-bp-gdpr');
				...
				$data = array(
					array("name"=>__('Group Name','buddyboss-bp-gdpr'), "value" => $item->name),
					array("name"=>__('Group Description','buddyboss-bp-gdpr'), "value" => $item->description),
					array("name"=>__('Group slug','buddyboss-bp-gdpr'), "value" => $item->slug),
					array("name"=>__('Created Date (GMT)','buddyboss-bp-gdpr'), "value" => $item->date_created),
					array("name"=>__('Group Status','buddyboss-bp-gdpr'), "value" => ucfirst($item->status)),
					array("name"=>__('Group Avatar','buddyboss-bp-gdpr'), "value" => $avatar),
					array("name"=>__('Group Cover Image','buddyboss-bp-gdpr'), "value" => $cover_photo),
					array("name"=>__('Group URL','buddyboss-bp-gdpr'), "value" => $group_permalink),
				);

				$metas2export = array();
				$metas2export["total_member_count"] = __("Total Members",'buddyboss-bp-gdpr');
				$metas2export["last_activity"] = __("Last Activity",'buddyboss-bp-gdpr');
				...
				foreach($item->metas as $meta) {
					if(isset($metas2export[$meta->meta_key])) {
						...
						$data[] = array("name"=>$metas2export[$meta->meta_key], "value" => $value);
					}
				}
		...
		function additional_group_items_delete($group_id) {
			...
			// delete cover image
			bp_attachments_delete_file( array( 'item_id' => $group_id, 'object_dir' => 'groups', 'type' => 'cover-image' ) );
